...
if not MAX_TOKEN:
    print("❌ КРИТИЧЕСКАЯ ОШИБКА: переменная MAX_BOT_TOKEN не найдена!")
    print("🔧 Добавьте переменную окружения на Bothost:")
    print("   Key: MAX_BOT_TOKEN")
    print("   Value: ваш_токен_от_MasterBot")
    exit(1)

# Информация о DeepSeek (необязательно)
if DEEPSEEK_KEY:
    print("✅ DeepSeek ключ загружен из переменных окружения")
else:
    print("ℹ️ DeepSeek не настроен (бот будет отвечать стандартно)")
...
def main():
    ...
    if not get_bot_info():
        ...
        log("🔧 Проверьте переменную окружения MAX_BOT_TOKEN на Bothost")
        return
