...
DEEPSEEK_KEY = "sk-1cd90d7386224b6e814d155b1442cf52"  # ваш реальный ключ
...
def ask_deepseek(user_message):
    ...
    """
    if not DEEPSEEK_KEY:
        return None  # DeepSeek не подключен
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log(f"✅ Ответ получен ({len(reply)} символов)")
            return reply
        else:
    ...
    for update in updates:
        if update.get('update_type') == 'message_created' and 'message' in update:
            ...
            if user_id and text:
                ...
                # Если DeepSeek не отвечает - используем простые ответы
                if not reply:
